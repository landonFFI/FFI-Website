...
## 3. Unsourced claim on `pages/outdoor-atm.html` (flagged, not changed)

The page says that roughly 25% of cash withdrawn at an ATM gets spent in or near the host store. It appears twice:

- About line 592: "Industry research shows roughly 25% of cash withdrawn at an ATM gets spent right inside the host store."
- About line 760: "roughly 25% of cash withdrawn at an ATM gets spent right inside the host store or nearby."

The page cites no source for it. It needs a source someone can check (publisher, study, and year). If no source can be found, remove both sentences. The copy is unchanged until Landon decides.
