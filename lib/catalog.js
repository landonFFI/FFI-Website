...
// Cassette prices come from the FFI Terminals spreadsheet, SWYPCO tab,
// "Sale Price (ACH)" column (synced September 23, 2026). Card buyers pay the
// same price; the sheet's higher "Sale Price (Card)" column is not used. Do not
// use the neighboring "Our Price (ACH)" column; that is wholesale cost.
...
const genmegaTopperGroup = (includedLabel) => ({
  key: 'topper',
  label: 'Topper',
  choices: [choice(includedLabel === 'Integrated' ? 'integrated' : 'none', includedLabel), choice('low', 'Low Topper', 60), choice('mini-high-bright', 'Mini High-Bright', 115)],
});
const printerGroup = { key: 'printer', label: 'Printer', choices: [choice('2in', '2-inch'), choice('3in', '3-inch', 150)] };

// ── CONFIRMED FFI SERVICE PRICING ───────────────────────────────────────────
// Programming and processing setup are Landon's real prices, not placeholders.
// They apply to every model, per machine.
const programmingGroup = {
  key: 'programming',
  label: 'Programming',
  taxCode: TAX_CODE_SERVICES,
  note: 'Self-programming comes with our free programming sheet and phone help from Ford Frontier Investments during business hours.',
  choices: [
    choice('self', 'Self-Program'),
    choice('factory', 'Factory Programmed', 150),
    choice('onsite', 'On-Site Technician', 250),
  ],
};
const processingGroup = {
  ...
  taxCode: TAX_CODE_SERVICES,
  note: 'Processing setup is free when you process with Ford Frontier Investments. Using a different processor adds a $500 setup fee.',
  choices: [
    choice('ffi', 'FFI Processing'),
    choice('outside', 'Outside Processor', 500),
  ],
};
const SERVICE_GROUPS = [programmingGroup, processingGroup];
...
// Photos are hotlinked from the manufacturers' own product pages. If one stops
// loading, the site shows a labeled "Photo coming soon" placeholder instead.
export const ATM_CATALOG = {
  'halo-ii': {
    ...
    cassettes: standardCassettes(2480, 2795, 2900, 3425),
    optionGroups: [lockGroup(350), keypadGroup, hyosungTopperGroup, nfcGroup(475), ...SERVICE_GROUPS],
  },
  force: {
    ...
    cassettes: [...standardCassettes(2695, 2960, 3065, 3640), cassette('6k', '6K', 4220)],
    optionGroups: [cardReaderGroup, lockGroup(350), hyosungTopperGroup, keypadGroup, cameraGroup, nfcGroup(475), ...SERVICE_GROUPS],
  },
  '2800t': {
    brand: 'Hyosung', model: '2800T', page: 'hyosung-2800t.html',
    image: 'https://hyosungamericas.com/wp-content/uploads/2023/06/Hyosung-MX2800T_front-e1689143782889.png',
    cassettes: [cassette('2k', '2K', 4555), cassette('4k', '4K', 5185), cassette('6k', '6K', 5815)],
    optionGroups: [cardReaderGroup, lockGroup(350), keypadGroup, cameraGroup, ...SERVICE_GROUPS],
  },
  g2500: {
    brand: 'Genmega', model: 'G2500', page: 'genmega-g2500.html',
    image: 'https://www.genmega.com/img/products/G2500/G2500_front.png',
    cassettes: [...standardCassettes(2535, 2800, 2905, 3480), cassette('6k', '6K', 4160), cassette('8k', '8K', 4840)],
    optionGroups: [
  ...
  onyx: {
    ...
    cassettes: [...standardCassettes(2695, 2960, 3065, 3640), cassette('6k', '6K', 4320), cassette('8k', '8K', 5000)],
    optionGroups: [
      lockGroup(360), genmegaTopperGroup('Integrated'),
  ...
  'onyx-w': {
    ...
    optionGroups: [lockGroup(360), nfcGroup(400), ...SERVICE_GROUPS],
  },
  nova: {
    ...
    optionGroups: [lockGroup(360), genmegaTopperGroup('No Topper'), nfcGroup(400), ...SERVICE_GROUPS],
  },
  ...
  gt300: {
    ...
    image: 'https://www.genmega.com/img/products/GT3000/GT3000_front.png',
    cassettes: [cassette('2k', '2K', 4630)],
    optionGroups: [
      lockGroup(360),
      { key: 'rear-panel', label: 'Rear Service Panel', choices: [choice('none', 'None'), choice('rear-panel', 'Rear Panel', 300)] },
      nfcGroup(400),
      ...SERVICE_GROUPS,
    ],
  },
  ...
  gt500: {
    ...
    image: 'https://www.genmega.com/img/products/GT5000/GT5000_front.png',
...
export function atmLineItems(item) {
  ...
  const lines = [priceDataLine(`${name}: ${cassetteChoice.label} Cassette`, cassetteChoice.amount, quantity, TAX_CODE_TANGIBLE_GOODS)];
  for (const { group, choice: picked } of options) {
    if (picked.amount > 0) {
      lines.push(priceDataLine(`${group.label}: ${picked.label} (${name})`, picked.amount, quantity, group.taxCode || TAX_CODE_TANGIBLE_GOODS));
    }
  }
  return lines;
}

function priceDataLine(name, unitAmount, quantity, taxCode) {
  return {
    quantity,
    price_data: {
      currency: 'usd',
